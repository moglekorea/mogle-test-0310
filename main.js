...
document.addEventListener('DOMContentLoaded', function() {
  ...
  fetch('config.json')
    ...
    .then(rawConfig => {
      ...
      // Doctor layout: hide empty slots, adjust columns
      (function adjustDoctorLayout() {
        var doctors = document.querySelectorAll('#doctors .doctor-card, #doctors .doctor-card-horizontal');
        var visibleCount = 0;
        doctors.forEach(function(card) {
          var nameEl = card.querySelector('.doctor-card__name, .doctor-card-horizontal__name');
        ...
        if (visibleCount > 0 && visibleCount < 3) {
          var visibleCols = [];
          document.querySelectorAll('#doctors .col').forEach(function(col) {
            if (col.style.display !== 'none') visibleCols.push(col);
          });
          var colClass = visibleCount === 1 ? 'col-12' : 'col-6';
          visibleCols.forEach(function(col) {
            col.className = col.className.replace(/col-\d+/g, '').trim() + ' ' + colClass;
          });
        }
      })();
    ...
    createButton: function() {
      if ($('.back-to-top').length === 0) {
        $('body').append(
          ...
          'background-color: #0ea5e9; color: #fff; border: none; border-radius: 50%; ' +
